...
export default function WebsitePreview({ mobileNumber, initialPath = 'index.html' }: WebsitePreviewProps) {
  ...
  useEffect(() => {
    const handleFileUpload = () => {
      if (mobileNumber) {
        fetchAvailableFiles();
        // Also refresh previews when files are uploaded
        refreshAllPreviews();
      }
    };
    ...
    const handleTemplateDeployment = () => {
      if (mobileNumber) {
        fetchAvailableFiles();
        // Refresh previews when template is deployed
        setMobileRefreshKey(prev => prev + 1);
        setDesktopRefreshKey(prev => prev + 1);
        
        // Show template deployment notification
        setRefreshNotification('Template deployed! Refreshing preview with new content...');
        setTimeout(() => setRefreshNotification(''), 5000);
      }
    };

    // Listen for custom upload events
    window.addEventListener('s3-upload-complete', handleFileUpload);
    window.addEventListener('template-deployed', handleTemplateDeployment);
    
    return () => {
      window.removeEventListener('s3-upload-complete', handleFileUpload);
      window.removeEventListener('template-deployed', handleTemplateDeployment);
    };
  }, [mobileNumber]);
  ...
  const saveFileContent = async () => {
    ...
    try {
      ...
      // Refresh both previews to show changes
      refreshAllPreviews();
  ...
  // Function to refresh desktop preview
  const refreshDesktopPreview = () => {
    setDesktopRefreshKey(prev => prev + 1);
  };

  // Function to refresh both previews
  const refreshAllPreviews = () => {
    setMobileRefreshKey(prev => prev + 1);
    setDesktopRefreshKey(prev => prev + 1);
    
    // Show refresh notification
    setRefreshNotification('Preview refreshed! Loading latest content...');
  ...
  return (
    <div className="bg-white shadow rounded-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="flex items-center px-2 py-1 text-sm text-indigo-600 hover:text-indigo-800 transition-colors"
              title="Refresh preview (use this if you see old content)"
            >
      ...
      <div className="p-6">
        ...
        {refreshNotification && (
          <div className="mb-4 p-4 bg-blue-50 rounded-lg flex items-start border border-blue-200">
            <svg className="animate-spin h-5 w-5 text-blue-500 mr-2 mt-0.5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="text-sm text-blue-700">{refreshNotification}</p>
          </div>
        )}
            ...
            <div className="bg-gray-50 p-3 rounded-md">
              ...
              <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded-md">
                <p className="text-xs text-yellow-700">
                  <strong>📝 Real-time Preview:</strong> This preview loads directly from S3 (bypassing CloudFront cache) to show your latest changes immediately. 
                  If you recently deployed a template but still see old content, use the "Refresh" button above. 
                  The preview will automatically refresh when you deploy templates or upload new files.
                </p>
              </div>
