...
export async function GET(
  ...
  try {
    ...
    // Remove the leading slash for joining with the base path
...
<body>
    <div class="container">
      ...
      // Use the same content type logic as below
      switch (ext) {
        case 'html':
    ...
    if (stats.isDirectory()) {
      ...
      if (existsSync(indexPath)) {
        ...
        // Add base path to make relative URLs work correctly
        const htmlContent = fileBuffer.toString('utf-8');
        const basePath = `/${mobileNumber}/`;
        const modifiedHtml = processHtml(htmlContent, basePath);
        
        return new NextResponse(Buffer.from(modifiedHtml, 'utf-8'), {
          headers: {
            'Content-Type': 'text/html',
            'Cache-Control': 'public, max-age=31536000',
          },
